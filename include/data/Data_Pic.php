...
class Data_Pic
{
	...
	private static $picConf = array(
		'' => array('width' => 2280, 'height' => 2280, 'force' => 1),
		'small' => array('width' => 120, 'height' => 120, 'force' => 3),
		'normal' => array('width' => 200, 'height' => 200, 'force' => 3),
		'middle' => array('width' => 480, 'height' => 480, 'force' => 3),
		'detail' => array('width' => 420, 'height' => 280, 'force' => 3),
		'big' => array('width' => 750, 'height' => 750, 'force' => 1),
	);
	...
	public static function savePic($picid, $name, $content, $basePath = OSS_PIC_PATH)
	{
		...
		if (! Conf_Base::isTestEnv()) {
            $ossName = File_Oss_Api::genOssFileNameForShi($filename);
            File_Oss_Api::uploadFileByPath($ossName, $desFile);
        } else {
//		    $ossName = File_Oss_Api::genOssFileNameForShi($filename);
//            $ossName = str_replace(File_Oss_Config::OSS_SHI_DIR_DF, File_Oss_Config::OSS_TEST_DIR_DF, $ossName);
//		    File_Oss_Api::uploadFileByPath($ossName, $desFile);
        }
	...
	public static function getPic($url, &$picid)
	{
		...
		if (!empty($info))
		{
			...
			$ret = Tool_Image::convertImage($srcFile, $info, $localFile,
							$picConf[$size]['width'],
							$picConf[$size]['height'],
							$picConf[$size]['force'], false, true, 0, true);
			if ($ret)
			{
				$content = file_get_contents($localFile);
				return $content;
	...
	{
		if (preg_match('@^\d+.\w+$@', $pictag)
			|| preg_match('@'.PIC_HOST.'\/pic\/@', $pictag))
		{
			return true;
		}
		return false;
